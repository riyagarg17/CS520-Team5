...
import { Card, Col, Row, Typography, Button, Modal, Form, Input, InputNumber, message } from "antd";
...
const Patient = () => {
    ...
    const handleFormSubmit = async (values) => {
        try {
            const response = await fetch(ENDPOINTS.predictObesityRisk, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                message.error('Failed to update health data');
            }
        } catch (error) {
            console.error('Error:', error);
            message.error('An error occurred while updating health data');
        }
    };
    ...
    return (
        <div className="patient-container">
            ...
            {riskLevel && (
                <Card className="risk-level-card" style={{ marginBottom: '20px' }}>
                    <Title level={4}>Obesity Risk Assessment</Title>
